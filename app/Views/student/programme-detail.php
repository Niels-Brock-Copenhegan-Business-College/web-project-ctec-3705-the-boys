<?php
// Default programme data if not provided by the controller.
    ...
    <div class="row">
      <div class="col-12">
        <?php if (!empty($prog['image_url'])): ?>
          <?php
            // Determine whether image_url is absolute or relative.
            $img = $prog['image_url'];
          ...
          <img
            ...
            alt="<?= htmlspecialchars($prog['title'], ENT_QUOTES) ?> programme image"
          >
        <?php endif; ?>
        ...
        <div class="lead" style="text-align: justify;">
          <?php foreach (explode("\n\n", trim($prog['description'])) as $para): ?>
            <p><?= nl2br(htmlspecialchars(trim($para), ENT_QUOTES)) ?></p>
          <?php endforeach; ?>
        </div>

        <a
          href="<?= base_url('/interest/register/' . $prog['id']) ?>"
          class="btn btn-primary btn-lg mt-2"
          aria-label="Register interest in <?= htmlspecialchars($prog['title'], ENT_QUOTES) ?>"
        >
          Register Interest
        </a>
      </div>
        ...
        <?php foreach ($modulesByYear as $year => $modules): ?>
          <div class="accordion-item mb-2 border rounded-3 overflow-hidden">
            <h3 class="accordion-header" id="heading-year<?= $year ?>">
              <button
                class="accordion-button <?= $year > 1 ? 'collapsed' : '' ?> fw-semibold"
                type="button"
            ...
            >
              <div class="accordion-body p-0">
                <?php foreach ($modules as $index => $m): ?>
                  <?php
                    // Build unique IDs and visual defaults for the module card.
                  ...
                  <div class="<?= $isLast ? '' : 'border-bottom' ?>">
                    <!-- Module header row -->
                    <div
                      class="d-flex align-items-center gap-3 px-3 py-3"
                    ...
                    >
                      <?php if ($hasPhoto): ?>
                        <img
                          src="<?= base_url('/uploads/' . htmlspecialchars($m['photo'], ENT_QUOTES)) ?>"
                          alt=""
                          aria-hidden="true"
                        ...
                        <?php if (!empty($shared)): ?>
                          <div class="d-flex flex-wrap gap-1 mt-1">
                            <?php foreach ($shared as $sp): ?>
                              <a
                                href="<?= base_url('/programmes/' . (int)$sp['id']) ?>"
                    ...
                    <div id="<?= $mid ?>" class="collapse">
                      <div class="px-3 pb-4 pt-1" style="padding-left:calc(3rem + 1.75rem + .5rem) !important;">
                        ...
                        <div class="text-muted" style="font-size:.9rem;line-height:1.7;text-align:justify;">
                          <?= htmlspecialchars($m['description'], ENT_QUOTES) ?>
                        </div>

                        <span class="badge mt-2" style="background:#f1f5f9;color:#475569;font-size:.75rem;">
                          <?= (int)($m['credits'] ?? 20) ?> credits
                        </span>
          ...
          <div class="col-sm-6 col-lg-4">
            <div class="d-flex flex-column p-4 bg-white rounded-3 border h-100" style="gap:.75rem;">
              ...
              <div class="d-flex align-items-center gap-3">
                ...
                <?php else: ?>
                  <div
                    style="width:3.2rem;height:3.2rem;border-radius:50%;background:linear-gradient(135deg,#003366,#00509e);
